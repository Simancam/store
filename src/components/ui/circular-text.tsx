...
const CircularText: React.FC<CircularTextProps> = ({
  ...
  return (
    <motion.div
      initial={{ rotate: 0 }}
      className={`mx-auto rounded-full w-[120px] h-[120px] text-white font-black text-center cursor-pointer origin-center ${className}`}
      animate={controls}
    ...
    >
      {letters.map((letter, i) => {
        ...
        return (
          <span
            key={i}
            className="absolute inline-block inset-0 text-xxs transition-all duration-500 ease-\[cubic-bezier\(0,0,0,1\)\]"
